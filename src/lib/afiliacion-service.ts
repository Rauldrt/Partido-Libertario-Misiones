...
export async function addAfiliacionSubmission(submission: Record<string, any>): Promise<void> {
    ...
    if (!db) {
        throw new Error("No se pudo enviar la afiliación: La base de datos de administrador no está inicializada.");
    }
    const afiliacionCollection = collection(db, 'afiliaciones');
